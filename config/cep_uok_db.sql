...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 15, 2026 at 08:17 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `generate_membership_number` (IN `p_member_id` INT, IN `p_membership_type_id` INT, IN `p_year_joined` YEAR)   BEGIN
    DECLARE v_type_code VARCHAR(10);
    DECLARE v_sequence INT;
    DECLARE v_membership_number VARCHAR(50);
    
    -- Get type code
    SELECT 
        CASE 
            WHEN type_name LIKE '%Student%' THEN 'STD'
            WHEN type_name LIKE '%Alumni%' THEN 'ALM'
            WHEN type_name LIKE '%Visitor%' THEN 'VST'
            WHEN type_name LIKE '%Donor%' THEN 'DNR'
            ELSE 'MBR'
    ...
    SELECT COALESCE(MAX(CAST(SUBSTRING_INDEX(membership_number, '-', -1) AS UNSIGNED)), 0) + 1
    INTO v_sequence
    FROM members
    WHERE membership_number LIKE CONCAT('CEP-', p_year_joined, '-', v_type_code, '-%');
    
    -- Generate membership number: CEP-YEAR-TYPE-SEQUENCE
    SET v_membership_number = CONCAT('CEP-', p_year_joined, '-', v_type_code, '-', LPAD(v_sequence, 4, '0'));
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_assign_role_to_user` (IN `p_user_id` INT, IN `p_role_id` INT, OUT `p_success` BOOLEAN, OUT `p_message` VARCHAR(255))   BEGIN
    ...
    ELSE
        ...
        SET p_success = TRUE;
        SET p_message = 'Role assigned successfully';
        
        COMMIT;
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_create_user` (IN `p_firstname` VARCHAR(100), IN `p_lastname` VARCHAR(100), IN `p_email` VARCHAR(255), IN `p_phone` VARCHAR(20), IN `p_password` VARCHAR(255), IN `p_role_id` INT, IN `p_created_by` INT, OUT `p_user_id` INT, OUT `p_success` BOOLEAN, OUT `p_message` VARCHAR(255))   BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        SET p_success = FALSE;
        SET p_message = 'Database error occurred';
    ...
    ELSE
        -- Generate username
        SET @username = LOWER(CONCAT(SUBSTRING(p_firstname, 1, 1), p_lastname));
        SET @counter = 1;
        
        WHILE EXISTS(SELECT 1 FROM users WHERE username = @username) DO
            SET @username = LOWER(CONCAT(SUBSTRING(p_firstname, 1, 1), p_lastname, @counter));
            SET @counter = @counter + 1;
        END WHILE;
        
        -- Insert user
        INSERT INTO users (firstname, lastname, username, email, phone, password, role_id, created_by, status)
        VALUES (p_firstname, p_lastname, @username, p_email, p_phone, p_password, p_role_id, p_created_by, 'pending');
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_get_user_with_permissions` (IN `p_user_id` INT)   BEGIN
    -- Get user basic info
    SELECT 
        u.*,
        r.name as role_name,
...
(1, 1, NULL, 'Super', 'Admin', 'admin', 'admin@cepuok.com', '+250788000000', '$2y$10$cTKQFPz493I5.QQkU1MwzOW.YLOdQKqnHbWzpsnO13eI54jLUnCt6', NULL, NULL, 1, NULL, NULL, NULL, NULL, '2026-02-15 17:04:38', 0, NULL, 'active', 1, 1, NULL, '2026-02-13 15:14:23', '2026-02-15 17:04:38', NULL);
...
-- Events
--
CREATE DEFINER=`root`@`localhost` EVENT `evt_cleanup_expired_sessions` ON SCHEDULE EVERY 1 HOUR STARTS '2026-02-13 17:14:23' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
